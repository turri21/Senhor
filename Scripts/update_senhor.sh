...
# ║                      CRT Display Mode                            ║
# ╚══════════════════════════════════════════════════════════════════╝
# Detected after terminal width is known (see below). Set to 'true'
# when running on a physical console with ≤72 visible columns — the
# typical safe area on a 240p/480i CRT via composite or S-Video.
# In CRT mode: block-font banners → compact text headers,
#              wide boxes → narrower boxes, 256-color → basic 8-color,
#              braille spinner → ASCII spinner, whiptail width clamped.
CRT_MODE=false
...
_read_mister_ini() {
    local ini_file=""
    # Prefer the active ini: check for MiSTer_*.ini alternatives first,
    # then fall back to the main MiSTer.ini
    for f in /media/fat/MiSTer.ini /media/fat/MiSTer_*.ini; do
        [[ -f "$f" ]] || continue
        ini_file="$f"
        # Parse the [menu] section only (stop at the next section header)
        local in_menu=false
        while IFS= read -r line; do
            ...
            line="${line%%\;*}"
            line="${line//[$'\t' ]/}"
            if [[ "$line" =~ ^\[([Mm][Ee][Nn][Uu])\]$ ]]; then
                in_menu=true; continue
            fi
            if [[ "$line" =~ ^\[.*\]$ ]]; then
                in_menu=false; continue
            fi
            if $in_menu; then
                if [[ "$line" =~ ^[Vv][Gg][Aa]_[Ss][Cc][Aa][Ll][Ee][Rr][[:space:]]*=[[:space:]]*([0-9]+) ]]; then
                    _ini_vga_scaler="${BASH_REMATCH[1]}"
                fi
                if [[ "$line" =~ ^[Ff][Bb]_[Ss][Ii][Zz][Ee][[:space:]]*=[[:space:]]*([0-9]+) ]]; then
                    _ini_fb_size="${BASH_REMATCH[1]}"
                fi
            fi
        done < "$ini_file"
        # Stop at the first ini file found (MiSTer.ini takes precedence)
        break
    done
}
_read_mister_ini
...
# Determine CRT_MODE
_current_tty=$(tty 2>/dev/null || echo "pts")
if [[ "${SENHOR_CRT:-0}" == "1" ]]; then
    CRT_MODE=true   # manual override
elif [[ "$_ini_vga_scaler" == "1" ]]; then
    CRT_MODE=true   # MiSTer.ini says scaler → VGA = CRT connected
elif [[ "$_current_tty" != *"pts"* ]] && [ "$_TERM_WIDTH" -le 76 ]; then
    CRT_MODE=true   # physical tty with narrow framebuffer (fallback)
fi

# fb_size-aware safe width and left margin
# fb_size=0 (auto) or 1 → full framebuffer, ~80 cols, safe area ~68
# fb_size=2           → pixel-doubled, ~40 cols, safe area ~34
# fb_size=3           → pixel-tripled, ~26 cols, safe area ~22
if $CRT_MODE; then
    case "$_ini_fb_size" in
        3) CRT_MAX_WIDTH=22; CRT_MARGIN=1 ;;
        2) CRT_MAX_WIDTH=34; CRT_MARGIN=1 ;;
        *) CRT_MAX_WIDTH=68; CRT_MARGIN=2 ;;  # fb_size 0 or 1
    esac
else
    CRT_MAX_WIDTH=9999
    CRT_MARGIN=0
...
if $CRT_MODE; then
    # Replace 256-color codes with safe basic-8 equivalents
    C_ORANGE="\e[1;33m"   # bold yellow — closest safe approximation
    C_PURPLE="\e[1;35m"   # bold magenta

    # Icon variables: plain ASCII for CRT (Unicode icons can smear on scanlines
    # or be absent when the custom PSF font hasn't loaded yet)
    ICO_OK="[OK]"
    ICO_ERR="[!!]"
    ICO_WARN="[!]"
    ICO_INFO="-->"
...
# crt_line: emit a single line respecting CRT_MARGIN and CRT_MAX_WIDTH.
# Strips ANSI escape codes when measuring length, so colour codes don't
# count toward the visible width. Usage: crt_line "text with ${COLOR}codes${C_RESET}"
crt_line() {
    local text="$1"
    if $CRT_MODE; then
        ...
        plain="${plain:0:$CRT_MAX_WIDTH}"
        printf "%${CRT_MARGIN}s" ""
        echo -e "${text:0:$(( ${#text} - ${#plain} + CRT_MAX_WIDTH ))}"
    else
        echo -e "$text"
    fi
}
...
clear
if $CRT_MODE; then
    _div=$(printf '%*s' "$CRT_MAX_WIDTH" '' | tr ' ' '=')
    echo -e "${C_CYAN}"
    crt_line "  ${_div}"
    crt_line "    S E N H O R   F P G A"
    crt_line "  ${_div}"
    echo -e "${C_RESET}"
    crt_line "${C_WHITE}${C_BOLD}  Update Senhor Script${C_RESET}  ${C_YELLOW}v${CURRENT_VERSION}${C_RESET}"
    crt_line "${C_DIM}  $(printf '%*s' "$CRT_MAX_WIDTH" '' | tr ' ' '-')${C_RESET}"
else
    echo -e "${C_CYAN}"
    cat << "EOF"                                                              
 ██████╗███████╗███╗   ██╗██╗  ██╗ ██████╗ █████╗            __           
...
# Show CRT detection result at boot (visible for ~2s then scroll off)
if $CRT_MODE; then
    _crt_src="tty/width"
    [[ "$_ini_vga_scaler" == "1" ]] && _crt_src="MiSTer.ini (vga_scaler=1)"
    [[ "${SENHOR_CRT:-0}" == "1" ]] && _crt_src="SENHOR_CRT=1 override"
    crt_line "${C_YELLOW}  ${ICO_WARN} CRT mode: ${_crt_src}, fb_size=${_ini_fb_size}, max_width=${CRT_MAX_WIDTH}${C_RESET}"
    echo
fi
...
#SPINNER_FRAMES
if $CRT_MODE; then
    # Replace braille spinner with plain ASCII — braille glyphs may smear on CRT
    SPINNER_FRAMES=('|' '/' '-' '\' '|' '/' '-' '\' '|' '/')
    _SPINNER_IDX=0
else 
    SPINNER_FRAMES=('⠋' '⠙' '⠹' '⠸' '⠼' '⠴' '⠦' '⠧' '⠇' '⠏')
...
print_dl_summary() {
    ...
    for (( i=start; i<count; i++ )); do
        echo -e "  ${C_GREEN}${ICO_DL}${C_RESET}  ${C_DIM}${files[$i]}${C_RESET}"
    done
...
draw_progress_bar() {
    ...
    # In CRT mode cap to the safe visible area for the current fb_size.
    local term_width="$_TERM_WIDTH"
    $CRT_MODE && term_width=$(( _TERM_WIDTH < CRT_MAX_WIDTH ? _TERM_WIDTH : CRT_MAX_WIDTH ))
    ...
    # Cap at sensible maxes — scale down for narrow CRT modes (fb_size=2/3)
    local _bar_max=28 _lbl_max=32
    if $CRT_MODE; then
        _bar_max=$(( CRT_MAX_WIDTH * 28 / 68 ))
        _lbl_max=$(( CRT_MAX_WIDTH * 32 / 68 ))
        [ "$_bar_max" -lt 6 ] && _bar_max=6
        [ "$_lbl_max" -lt 6 ] && _lbl_max=6
    fi
    [ "$bar_width" -gt "$_bar_max" ]   && bar_width="$_bar_max"
    [ "$label_width" -gt "$_lbl_max" ] && label_width="$_lbl_max"
    ...
    local tag=""
    local _ico_ex; $CRT_MODE && _ico_ex="*" || _ico_ex="◈"
    case "$tag_key" in
        DL)   tag="${C_GREEN}[${ICO_DL} DL]${C_RESET}" ;;
        EX)   tag="${C_CYAN}[${_ico_ex} EX]${C_RESET}" ;;
        SKIP) tag="${C_DIM}[-- ]${C_RESET}" ;;
        ERR)  tag="${C_RED}[${ICO_ERR} !!]${C_RESET}" ;;
        *)    tag="      " ;;
    esac
...
check_disk_space() {
    ...
    if [ -n "$avail_mb" ] && [ "$avail_mb" -lt "$min_mb" ]; then
        echo
        if $CRT_MODE; then
            local _div; _div=$(printf '%*s' "$CRT_MAX_WIDTH" '' | tr ' ' '=')
            local _divd; _divd=$(printf '%*s' "$CRT_MAX_WIDTH" '' | tr ' ' '-')
            crt_line "${C_YELLOW}  ${_div}${C_RESET}"
            crt_line "${C_YELLOW}  ${ICO_WARN} Low Disk Space Warning${C_RESET}"
            crt_line "${C_YELLOW}  ${_divd}${C_RESET}"
            crt_line "${C_YELLOW}  Only ${avail_mb}MB free on $target${C_RESET}"
            crt_line "${C_YELLOW}  Recommended: ${min_mb}MB+ free${C_RESET}"
            crt_line "${C_YELLOW}  Downloads may fail or be incomplete.${C_RESET}"
            crt_line "${C_YELLOW}  ${_div}${C_RESET}"
        else
...
log() {
    ...
    case "$level" in
        ERROR)   crt_line "${C_RED}  ${ICO_ERR}  $msg${C_RESET}" ;;
        WARN)    crt_line "${C_YELLOW}  ${ICO_WARN}  $msg${C_RESET}" ;;
        SUCCESS) crt_line "${C_GREEN}  ${ICO_OK}  $msg${C_RESET}" ;;
        *)       crt_line "${C_BLUE}  ${ICO_INFO}  $msg${C_RESET}" ;;
    esac
}
...
check_internet() {
    ...
    if [ "$connected" = true ]; then
        ...
        if $CRT_MODE; then
            ...
            local _divd; _divd=$(printf '%*s' "$CRT_MAX_WIDTH" '' | tr ' ' '-')
            echo -e "${C_RED}"
            # Simple ASCII antenna — short enough even for fb_size=2 (34 cols)
            crt_line "       ____"
            crt_line "      /    \\"
            crt_line "     | WiFi |  ~~~  ???"
            crt_line "      \\____/"
            echo
            echo -e "${C_RESET}"
            crt_line "${C_RED}  ${_div}${C_RESET}"
            crt_line "${C_RED}  ${ICO_ERR} No Internet Connection${C_RESET}"
            crt_line "${C_RED}  ${_divd}${C_RESET}"
            crt_line "  - Check WiFi/Ethernet cable"
            crt_line "  - Verify DNS (8.8.8.8 / 1.1.1.1)"
            crt_line "  - Check for a firewall"
            crt_line "  - Try a webpage in a browser"
            crt_line "${C_RED}  ${_div}${C_RESET}"
        else
            echo -e "                  ████████████████████                                         "
            echo -e "                  ████  ██████████████████                                     "
...
display_news() {
    ...
    if $CRT_MODE; then
        local _div; _div=$(printf '%*s' "$CRT_MAX_WIDTH" '' | tr ' ' '=')
        crt_line "${C_WHITE}  ${_div}${C_RESET}"
        crt_line "${C_WHITE}  *  SENHOR NEWS${C_RESET}"
        crt_line "${C_WHITE}  ${_div}${C_RESET}"
    else
    ...
    echo
    echo -e "${C_MAGENTA}  ${ICO_PROMPT} Press any key to continue...${C_RESET}"
    read -n 1 -s -r
    echo
    return 0
}
...
download_gbaborders() {
    ...
    echo -e "${C_CYAN}  ║${C_RESET}  Downloaded  ${C_GREEN}${ICO_DL}${C_RESET}    : ${C_GREEN}${C_BOLD}$DOWNLOADED_FILES${C_RESET}                 ${C_CYAN}║${C_RESET}"
    echo -e "${C_CYAN}  ║${C_RESET}  Up to date  ${C_DIM}─${C_RESET}    : ${C_DIM}$SKIPPED_FILES${C_RESET}               ${C_CYAN}║${C_RESET}"
    echo -e "${C_CYAN}  ║${C_RESET}  Failed      ${C_RED}${ICO_ERR}${C_RESET}    : ${C_RED}$FAILED_FILES${C_RESET}                 ${C_CYAN}║${C_RESET}"
    echo -e "${C_CYAN}  ╠═══════════════════════════════════════╣${C_RESET}"
...
main() {
    ...
    # In CRT mode cap width to stay within the safe visible area
    local _wt_width=74
    $CRT_MODE && _wt_width=$(( CRT_MAX_WIDTH < 68 ? CRT_MAX_WIDTH : 68 ))
    CHOICES=$(whiptail --title "Senhor Downloader" --checklist \
        "Choose what you want to download (use space to select):" 27 "$_wt_width" 19 \
        "RBF_MGL" "DL RBF/MGL files$(show_update_info "RBF_MGL")" ON \
        "MRA" "DL MRA files$(show_update_info "MRA")" OFF \
        "Menu" "DL Menu$(show_update_info "Menu")" OFF \
        "MiSTer_binary" "DL MiSTer bin for Senhor$(show_update_info "MiSTer_binary")" OFF \
        "Linux" "DL various files for Linux$(show_update_info "Linux")" OFF \
        "Alternatives" "DL Alternative MRA files$(show_update_info "Alternatives")" OFF \
        "ArcadeROMs" "DL Arcade ROMs [SLOW]$(show_update_info "ArcadeROMs")" OFF \
        "BIOS" "DL BIOS files [SLOW]$(show_update_info "BIOS")" OFF \
        "Cheats" "DL Cheats$(show_update_info "Cheats")" OFF \
        "Docs" "DL Docs$(show_update_info "Docs")" OFF \
        "Filters" "DL Filters$(show_update_info "Filters")" OFF \
        "Filters_Audio" "DL Filters_Audio$(show_update_info "Filters_Audio")" OFF \
        "Fonts" "DL Fonts$(show_update_info "Fonts")" OFF \
        "Gamma" "DL Gamma$(show_update_info "Gamma")" OFF \
        "GBA_Borders" "DL GBA_Borders$(show_update_info "GBA_Borders")" OFF \
        "Presets" "DL Presets$(show_update_info "Presets")" OFF \
        "Scripts" "DL Scripts$(show_update_info "Scripts")" OFF \
        "Shadow_Masks" "DL Shadow Masks$(show_update_info "Shadow_Masks")" OFF \
        "Wallpapers" "DL Various Wallpapers$(show_update_info "Wallpapers")" OFF \
        3>&1 1>&2 2>&3)

    # User cancelled
    if [ $? -ne 0 ]; then
        echo -e "${C_YELLOW}  ${ICO_WARN}  Operation cancelled by user. Goodbye!${C_RESET}"
        exit 1
    fi
...
if $CRT_MODE; then
    _div=$(printf '%*s' "$CRT_MAX_WIDTH" '' | tr ' ' '=')
    _divd=$(printf '%*s' "$CRT_MAX_WIDTH" '' | tr ' ' '-')
    echo -e "${C_CYAN}"
    crt_line "  ${_div}"
    crt_line "    S E N H O R   F P G A"
    crt_line "  ${_div}"
    echo -e "${C_RESET}"
    crt_line "${C_WHITE}  ${_divd}${C_RESET}"
    crt_line "${C_WHITE}  ${ICO_DL} Download session started...${C_RESET}"
    crt_line "${C_WHITE}  ${_divd}${C_RESET}"
else
    echo -e "${C_CYAN}"
    cat << "EOF"
...
EOF
    echo -e "${C_RESET}"
    echo -e "${C_WHITE}  ═════════════════════════════════════════════════════════════════${C_RESET}"
    echo -e "${C_WHITE}     ${ICO_ARROW}  Download session started  ·  please wait                   ${C_RESET}"
    echo -e "${C_WHITE}  ═════════════════════════════════════════════════════════════════${C_RESET}"
fi
...
DONE
    ...
    if $CRT_MODE; then
        local _div; _div=$(printf '%*s' "$CRT_MAX_WIDTH" '' | tr ' ' '=')
        local _divd; _divd=$(printf '%*s' "$CRT_MAX_WIDTH" '' | tr ' ' '-')
        crt_line "${_summary_color}  ${_div}${C_RESET}"
        crt_line "${_summary_color}    Session Summary${C_RESET}"
        crt_line "${_summary_color}  ${_div}${C_RESET}"
        crt_line "  Status    : ${_summary_color}${_summary_status}${C_RESET}"
        crt_line "  New Cores : ${C_GREEN}${SESSION_TOTAL_CORES} core(s)${C_RESET}"
        crt_line "  Errors    : ${C_RED}${SESSION_TOTAL_ERR}${C_RESET}"
        crt_line "  Duration  : ${SESSION_MINS}m ${SESSION_SECS}s"
        crt_line "${_summary_color}  ${_divd}${C_RESET}"
        crt_line "${_summary_color}  Safe to power off your Senhor.${C_RESET}"
        crt_line "${_summary_color}  ${_div}${C_RESET}"
    else
    ...
    if [ "${#SESSION_ERRORS[@]}" -gt 0 ]; then
        ...
        for err in "${SESSION_ERRORS[@]}"; do
            echo -e "  ${C_RED}${ICO_ERR}${C_RESET}  ${C_DIM}${err}${C_RESET}"
        done
    fi
    echo
}
